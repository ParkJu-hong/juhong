...
const Tweets = () => {
  ...
  const handleButtonClick = (event) => {
    ...
    const resultTemp = {
      ...
      content: msg,
      createdAt: `${now.getFullYear()} ${now.getMonth()} ${now.getDay()}`,
    updatedAt: `${now.getFullYear()} ${now.getMonth()} ${now.getDay()}`
    }

    let result = [...tweet];
    result.unshift(resultTemp); // 객체만 넣어줘도 됨

    setTweet(result); // tweet = result 라는 뜻....
  };
  ...
  return (
    <React.Fragment>
      ...
      <ul className="tweets">
        ...
        {tweet.map((el) => <Tweet key={el.id} tweet={el}/>)}
      </ul>
      <Footer />
    </React.Fragment>
  );
